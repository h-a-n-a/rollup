...
export default function umd(
  ...
  const defaultExport =
    exportMode === 'default' ? `global.${options.moduleName} = ` : ''

  const intro = `(function (global, factory) {
    ...
    ', '
		...
		`
    .replace(/^\t\t/gm, '')
    .replace(/^\t/gm, magicString.getIndentString())
